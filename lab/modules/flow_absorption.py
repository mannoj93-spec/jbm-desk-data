...
Point-in-time contract (lab-2.0, see lab/asof.py). A window's REQUIRED inputs are every perp bar
the features read: the window, the prior 60 minutes (sigma and prior return), the prior 24 h of
windows (volume median) and the 7-day history behind the hourly threshold. Their latest
availability is t_inputs; a late bar anywhere in that range delays the decision, and more than
60 minutes of delay excludes it. Spot flow, the settled funding rate and the depth snapshot are
OPTIONAL: each is used only if it was already available at t_inputs, otherwise it is None.
"""
import math

from lab.asof import Known, decide, grid
from lab.common import BASIS_PROSPECTIVE, BASIS_RECONSTRUCTION, DAY, H, MINUTE, hash_inputs
...
def build(bars_perp, bars_spot, start, end, params, basis, snaps=(), funding=(), code=""):
    """Returns (events, controls, coverage). Pure function of its inputs. `funding` is a list of
    (t, rate, avail); `snaps` are snapshot rows carrying observed_at."""
    q, weak_max, strong_min = params["flow_quantile"], params["weak_max"], params["strong_min"]
    first = (start // STEP) * STEP
    ts, perp, rm = grid(bars_perp, first, end)
    _, spot, rm_spot = grid(bars_spot, first, end)
    n = len(ts)
    lr = [math.log(b["c"] / b["o"]) if b else None for b in perp]      # within-bar return (open->close)
    events, controls = [], []
    flow_hist, vol5_hist, spot_vol5_hist = [], [], []
    threshold, thr_at, thr_avail = None, None, 0
    cov = {"steps": 0, "skipped_missing": 0, "skipped_history": 0, "late_inputs": 0, "events": 0, "controls": 0,
           "spot_not_yet_available": 0}
    fund = Known(funding)
    snap_k = Known([(s["t"], s, s.get("observed_at")) for s in snaps])
    hist_bars = 7 * 288 * 5 + 1440 + 70                                  # bars behind the threshold history
    for i in range(65, n):
        ...
        if thr_at is None or t_close - thr_at >= H:
            ...
            thr_at = t_close
            thr_avail = rm.query(i - hist_bars, i - 4)                      # everything the history read
        flow_hist.append(flow_norm)
        # required inputs: window + prior hour + 24 h volume history (+ threshold history)
        t_inputs = max(rm.query(i - 1444, i + 1), thr_avail if threshold is not None else 0)
        t_available, excluded = decide(t_close, t_inputs)
        prior_ret = direction * math.log(win[0]["o"] / perp[i - 64]["o"]) if perp[i - 64] else None
        f = fund.latest(t_close, t_inputs)
        funding_last = f[1] if f else None
        # optional spot flow: only if every spot bar it reads was available by t_inputs
        spot_ok_asof = spot_ok and smed and rm_spot.query(i - 1444, i + 1) <= t_inputs
        if spot_ok and not spot_ok_asof:
            cov["spot_not_yet_available"] += 1
        half_spread = depth50 = None
        sn = snap_k.latest(t_close, t_inputs, min_t=t_close - 15 * MINUTE)
        if sn:
            d = sn[1].get("depth_binance_usdt") or {}
            if d.get("st") == "ok" and d.get("best_bid"):
        ...
        feats = {"nti_btc": round(nti, 4), "flow_norm": round(flow_norm, 4), "resp_sigma": round(resp, 4),
                 ...
                 "spot_flow_norm": round(spot_nti / smed, 4) if spot_ok_asof else None,
                 ...
                 "threshold": threshold, "severity": abs(flow_norm)}
        quality = {"spot": "ok" if spot_ok_asof else ("late" if spot_ok else "missing"),
                   "spread": "snapshot" if half_spread is not None else "absent"}
        in_hash = hash_inputs([[b["t"], b["o"], b["c"], b["v"], b["tbv"]] for b in win])
        if excluded:
            cov["late_inputs"] += 1
            continue
        if t_close % H == 0:
            controls.append(event_record(ID + ":control", VERSION, t_close, win[0]["avail"], t_available, direction,
        ...
        if threshold is not None and abs(flow_norm) >= threshold:
            ...
            events.append(event_record(ID, VERSION, t_close, min(b["avail"] for b in win), t_available, direction,
                                       group, feats, in_hash, basis, {"window_bars": 5}, quality, code,
                                       t_inputs=t_inputs))
            cov["events"] += 1
...
def run(lab, params):
    """Prospective (as-of replay) pass over stored bars, and a reconstruction pass when history was fetched."""
    ...
    funding = lab.store.funding_known()
    if stored_perp:
    ...
    if lab.history.get("binance_klines_1m_BTCUSDT_perp"):
        ...
        fund = [(t, r, t) for t, r, _ in funding]              # reconstruction: availability ASSUMED at settlement
        ev, ctl, cov = build(hp, hs, start, end, params, BASIS_RECONSTRUCTION, (), fund, lab.code)
